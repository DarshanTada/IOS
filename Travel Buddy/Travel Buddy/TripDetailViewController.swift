...
//        showRoute()
    ...
    func showRoute() {
        ...
        directions.calculate { response, error in
            ...
            if let route = response.routes.first {
                ...
                self.zoomToRoute(route)
            }
        }
    }
    

    // MARK: - Zooming to Route
    func zoomToRoute(_ route: MKRoute) {
    ...
    @IBAction func btnExpensesClicked(_ sender: UIButton) {
        self.redirectToTripExpenses(tripValue: self.trip)
    }
    
    func redirectToTripExpenses(tripValue: Trip?) {
        let storyBoard = UIStoryboard(name: "Main", bundle: nil)
        let viewController = storyBoard.instantiateViewController(withIdentifier: "TripExpensesViewController") as! TripExpensesViewController
        viewController.selectedTrip = tripValue
